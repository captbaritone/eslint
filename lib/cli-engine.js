...
CLIEngine.prototype = {
    ...
    executeOnFiles: function(patterns) {
        ...
        function executeOnFile(filename, warnIgnored) {
            var hashOfConfig;
            var resolvedFilename = path.resolve(options.cwd, filename);

            if (options.ignore !== false) {

                if (ignoredPaths.contains(resolvedFilename, "custom")) {
                    if (warnIgnored) {
                        results.push(createIgnoreResult(resolvedFilename));
                    }
                    return;
                }

                if (ignoredPaths.contains(resolvedFilename, "default")) {
                    return;
                }

            }

            if (processed[resolvedFilename]) {
                return;
            }
            ...
            if (options.cache) {
                ...
                var descriptor = fileCache.getFileDescriptor(resolvedFilename);
                var meta = descriptor.meta || {};

                hashOfConfig = hashOfConfigFor(resolvedFilename);

                var changed = descriptor.changed || meta.hashOfConfig !== hashOfConfig;
                if (!changed) {
                    debug("Skipping file since hasn't changed: " + resolvedFilename);
                    ...
                    processed[resolvedFilename] = true;
            ...
            debug("Processing " + resolvedFilename);

            processed[resolvedFilename] = true;

            var res = processFile(resolvedFilename, configHelper, options);
            ...
            if (options.cache) {
                ...
                if ( res.errorCount > 0 || res.warningCount > 0 ) {
                    debug("File has problems, skipping it: " + resolvedFilename);
                    // remove the entry from the cache
                    fileCache.removeEntry( resolvedFilename );
                } else {
    ...
    executeOnText: function(text, filename) {
        ...
        // filename must be absolute for ignoredPaths.contains
